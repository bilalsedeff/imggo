...
BEGIN
  ...
  -- Get the target version data from pattern_versions (including csv_delimiter)
  SELECT
    json_schema,
    yaml_schema,
    xml_schema,
  ...
  -- Update patterns table with the target version's data (including csv_delimiter)
  ...
  SET
    ...
    csv_delimiter = v_version_data.csv_delimiter,
...
COMMENT ON FUNCTION public.switch_to_pattern_version IS 'Switch a pattern to use a specific version from its version history (with CSV delimiter support)';
